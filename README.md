...
The deployment uses the official Confluent Helm chart, combined with custom-built operator and init-container images that are compiled for the s390x architecture.
...
OpenShift SCC is fully compatible because runAsUser is not enforced (we removed it from the values file).
...
Disables features you are not using (webhooks, telemetry, managed certs)
...
No init-container will appear, because the official CFK chart does not use one.
